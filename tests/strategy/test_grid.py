import pytest
from src.strategy.grid import AdaptiveGrid, GridState, GridLevel, GridInput
...
def test_generate_basic_grid():
    ...
    state = grid.generate(GridInput(mid_price=mid_price, atr_value=atr_value, base_size=1.0))
...
def test_generate_zero_spacing_fallback():
    ...
    state = grid.generate(GridInput(mid_price=mid_price, atr_value=atr_value))
...
def test_generate_positive_bias():
    ...
    state = grid.generate(GridInput(mid_price=100.0, atr_value=2.0, bias_score=1.0, base_size=1.0))
...
def test_generate_trend_regime_amplifies_bias():
    ...
    range_state = grid.generate(
        GridInput(mid_price=100.0, atr_value=2.0, bias_score=1.0, base_size=1.0, regime="range")
    )

    # Trend regime
    trend_state = grid.generate(
        GridInput(mid_price=100.0, atr_value=2.0, bias_score=1.0, base_size=1.0, regime="trend")
    )
...
def test_distance_decay():
    ...
    state = grid.generate(GridInput(mid_price=100.0, atr_value=2.0, base_size=1.0))
...
def test_needs_rebalance():
    ...
    grid.generate(GridInput(mid_price=100.0, atr_value=2.0))
